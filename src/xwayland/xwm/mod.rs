...
impl Drop for IncomingTransfer {
    fn drop(&mut self) {
        ...
        if self.token.is_some() {
            println!("WARNING: IncomingTransfer freed before being removed from EventLoop");
        }
    }
}
...
impl Drop for OutgoingTransfer {
    fn drop(&mut self) {
        if self.token.is_some() {
            println!("WARNING: OutgoingTransfer freed before being removed from EventLoop");
        }
    }
}
...
impl XWmSelection {
    fn new(
        ...
        slog::debug!(
            log,
            "Selection init (Window: {:?}) for selection {:?} (atom: {:?})",
            window,
            selection,
            atom
        );
...
impl X11Wm {
        ...
        L: Into<Option<::slog::Logger>>,
    {
        ...
        let log = crate::slog_or_fallback(log).new(slog::o!("smithay_module" => "xwayland", "XWm" => id.0));
        let screen = 0;
        ...
        let clipboard = XWmSelection::new(&conn, &screen, &atoms, atoms.CLIPBOARD, &log)?;
        let primary = XWmSelection::new(&conn, &screen, &atoms, atoms.PRIMARY, &log)?;
        ...
        D: XwmHandler + 'static,
    {
        ...
        slog::info!(
            self.log,
            "Send request for {:?} from XWayland for {:?}",
            selection.type_,
            mime_type,
        );
        ...
        slog::info!(self.log, "Mime-type {:?} / Atom {:?}", mime_type, atom);
        ...
        if let Err(err) = fcntl(
            ...
            slog::warn!(self.log, "Failed to restrict wl file descriptor: {:?}", err);
        }
        ...
        let token = loop_handle
            .insert_source(
                ...
                move |_, fd, data| {
                    ...
                    {
                        match write_selection_callback(fd.as_fd(), conn, atoms, transfer, log) {
    ...
    match event {
        ...
        Event::SelectionRequest(n) => {
            ...
            if n.requestor == selection.window {
                slog::warn!(xwm.log, "Got SelectionRequest from our own selection window..");
                send_selection_notify_resp(&conn, &n, false)?;
                return Ok(());
            }
            ...
            if selection.window != n.owner {
                if n.time != x11rb::CURRENT_TIME && n.time < selection.timestamp {
                    slog::warn!(
                        xwm.log,
                        "Ignoring request with old timestamp {}, expected at least {}",
                        n.time,
                        selection.timestamp
                    );
                    send_selection_notify_resp(&conn, &n, false)?;
                ...
                match n.target {
                    x if x == xwm.atoms.TARGETS => {
                        ...
                        let targets = [xwm.atoms.TARGETS, xwm.atoms.TIMESTAMP]
                            ...
                            .collect::<Vec<u32>>();
                        slog::trace!(xwm.log, "Sending TARGETS to {}: {:?}", n.requestor, targets);
                        conn.change_property32(
                            PropMode::REPLACE,
                            n.requestor,
                            n.property,
                    ...
                    target => {
                        let mime_type = match target {
                            ...
                            x => {
                                let Some(mime) = conn.get_atom_name(x)?.reply_unchecked()?.and_then(|reply| String::from_utf8(reply.name).ok()) else {
                                    slog::debug!(xwm.log, "Unable to determine mime type from atom: {}", x);
                                    send_selection_notify_resp(&conn, &n, false)?;
                                    return Ok(());
                                };
                                ...
                                if !selection.mime_types.contains(&mime) {
                                    slog::warn!(
                                        xwm.log,
                                        "Mime type requested by Xwayland ({}) not offered",
                                        mime
                                    );
                                    send_selection_notify_resp(&conn, &n, false)?;
                                    return Ok(());
                            ...
                            move |_, fd, data| {
                                    ...
                                    .find(|t| t.request.requestor == requestor)
                                {
                                    match read_selection_callback(
                                        ...
                                        Ok(_) => {}
                                        Err(err) => {
                                            slog::warn!(xwm.log, "transfer aborted: {}", err);
                                        }
                                    };
                        ...
                        slog::debug!(
                            xwm.log,
                            "Created outgoing transfer from {:?} to {} with type {:?}",
                            selection.type_,
                            n.requestor,
                            mime_type,
                        );
            ...
            if n.state == Property::NEW_VALUE && n.atom == xwm.atoms._WL_SELECTION {
                if let Some(selection) = if xwm.clipboard.incoming.iter().any(|t| t.window == n.window) {
                    ...
                    if transfer.incr {
                        ...
                        {
                            if prop.value_len == 0 {
                                slog::debug!(xwm.log, "Incr Transfer complete! {:?}", transfer);
            ...
            if n.state == Property::DELETE {
                    ...
                    .any(|t| t.incr && t.request.requestor == n.window && t.request.property == n.atom)
                {
                    ...
                    if transfer.flush_property_on_delete {
                        ...
                        slog::trace!(xwm.log, "Send data chunk to {}: {}", requestor, len);
                        ...
                        .find(|x| x.window_id() == msg.window || x.mapped_window_id() == Some(msg.window))
                    {
                        ...
                        match wl_surface {
                            ...
                            Ok(wl_surface) => {
                                let log = xwm.log.clone();
                                let surface = surface.clone();
                                X11Wm::new_surface(state, xwm_id, surface, wl_surface, log);
                            }
                        }
                    }
                }
        ...
        Event::Error(err) => {
            slog::info!(xwm.log, "Got X11 Error: {:?}", err);
        }
        _ => {}
    }
    conn.flush()?;
...
fn read_selection_callback(
    ...
    let Ok(len) = nix::unistd::read(fd.as_raw_fd(), &mut buf) else {
        slog::debug!(log, "File descriptor closed, aborting transfer to {}", transfer.request.requestor);
        send_selection_notify_resp(conn, &transfer.request, false)?;
        return Ok(OutgoingAction::Done);
    };
    slog::trace!(
        log,
        "Transfer to {} became readable, read {} bytes",
        transfer.request.requestor,
        len
    );
    ...
    if transfer.source_data.len() >= INCR_CHUNK_SIZE {
        if !transfer.incr {
            ...
            slog::trace!(
                log,
                "Transfer to {} became incremental",
                transfer.request.requestor,
            );
            conn.change_property32(
                PropMode::REPLACE,
    ...
    if len == 0 {
        if transfer.incr {
            slog::debug!(log, "Incr transfer completed");
            ...
            let len = transfer.flush_data()?;
            slog::debug!(log, "Non-Incr transfer completed with {} bytes", len);
            send_selection_notify_resp(conn, &transfer.request, true)?;
    ...
    match transfer.write_selection(fd) {
        Ok(true) => {
            if transfer.incr {
                ...
                slog::debug!(log, "Non-Incr Transfer complete! {:?}", transfer);
                Ok(IncomingAction::Done)
            }
        }
        Ok(false) => Ok(IncomingAction::WaitForWritable),
        Err(err) => {
            slog::warn!(log, "Transfer errored: {:?}", err);
            if transfer.incr {
