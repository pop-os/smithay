...
lazy_static::lazy_static! {
    pub(crate) static ref DISPLAYS: Mutex<HashMap<u64, Weak<EGLDisplayHandle>>> = Mutex::new(HashMap::new());
}
...
unsafe fn select_platform_display<N: EGLNativeDisplay + 'static>(
...
) -> Result<(Arc<EGLDisplayHandle>, &'static str), Error> {
    for platform in native.supported_platforms() {
        ...
        displays.retain(|_, disp| disp.upgrade().is_some());

        let name = platform.platform_name;
        let handle = match displays.entry(hash) {
            Entry::Vacant(e) => {
                ...
                let handle = Arc::new(EGLDisplayHandle {
                    handle: display,
                    should_terminate: true,
                    _native: Box::new(native) as Box<dyn std::any::Any + 'static>,
                });

                e.insert(Arc::downgrade(&handle));

                handle
            }
            Entry::Occupied(e) => e.get().upgrade().unwrap(),
        };

        info!("Successfully selected EGL platform: {}", name);
